...
def main():
    ...
    with st.sidebar:
        ...
        default_model = get_secret("OLLAMA_MODEL", "llama3")
        ...
        num_hypotheses = st.slider("Number of Hypotheses", 1, 5, 3)
        
        include_tte = st.checkbox("Include TTE Components", value=True, 
                                  help="Target Trial Emulation framework")
        include_bias = st.checkbox("Include Bias Mitigation", value=True,
                                   help="Negative control, confounders")
        include_feasibility = st.checkbox("Include Feasibility Check", value=True,
                                          help="Data source suitability")
        
        focus_areas = st.multiselect(
            "Focus Areas",
            ["Comparator Gaps", "Population Gaps", "Outcome Gaps", "Temporal Gaps"],
            default=["Comparator Gaps", "Population Gaps"]
        )
        ...
        if api_key and email:
            st.success("✅ NCBI Credentials")
        else:
            st.error("❌ Missing NCBI")
        
        # Ollama Cloud Status
        if ollama_api_key:
            st.success("✅ Ollama API Key")
        else:
            st.error("❌ Missing Ollama Key")
        
        st.divider()
        st.caption("[NCBI API Key](https://www.ncbi.nlm.nih.gov/account/) | [Ollama Key](https://ollama.com/settings/keys)")
    ...
    if st.button("🚀 Analyze Trends & Generate Hypotheses", type="primary"):
        ...
        with st.status("Processing...", expanded=True) as status:
            ...
            # 2. Analyze
            try:
                # Get prompt options from session state
                prompt_options = st.session_state.get('prompt_options', None)
                result = analyzer.analyze_and_hypothesize(abstracts, topic_input, prompt_options)
                
                # Check result validity
                if result:
                    st.write(f"✅ Landscape summary: {len(result.landscape_summary)} chars")
                    st.write(f"✅ Gaps: {len(result.identified_gaps)} | Hypotheses: {len(result.hypotheses)}")
